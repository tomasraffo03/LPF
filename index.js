...
app.get('/home', (req, res) => {
  ...
  connection.query("SELECT * FROM user WHERE user_username = ?", [req.session.username], (err, rows) => {
    if (err) { throw err }
    connection.query("SELECT match_id, match_date, match_hour, (SELECT COUNT(team_id) FROM team WHERE team_id = match_team1 OR team_id = match_team2) AS playersCount FROM matchs WHERE match_status = ? OR match_status = ?", ['pendiente', 'confirmado'], (err, rowsMatches) => {
      if (err) throw err;
...
app.get('/partido/:idPartido', (req, res) => {
  connection.query("SELECT * FROM matchs LEFT JOIN match_det ON matchs.match_id = match_det.match_det_id WHERE match_id = ?", [req.params.idPartido], (err, rowsMatch) => {
    ...
    if (rowsMatch.length > 0) {
      connection.query("SELECT team.team_player, team.team_id, user.user_name, user.user_surname, (SELECT COUNT(team_id) FROM team WHERE team_id = team_id) AS playerCount FROM team INNER JOIN user ON team.team_player = user.user_id WHERE team.team_id = ? OR team.team_id = ?", [rowsMatch[0].match_team1, rowsMatch[0].match_team2], (err, rowsTeams) => {
        if (err) { throw err }
...
app.get('/cargarResultado/:idPartido', (req, res) => {
  if (req.session.role == 'A') {
    connection.query("SELECT * FROM matchs LEFT JOIN match_det ON matchs.match_id = match_det.match_det_id WHERE match_id = ?", [req.params.idPartido], (err, rowsMatch) => {
      if (err) { throw err }
      if (rowsMatch.length > 0) {
